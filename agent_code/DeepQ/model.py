...
class Qcnn(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.dropout2(x)
        x = HYPER.activation_function(self.hidden1(x))
        
        x = HYPER.activation_function(self.hidden2(x))
        x = HYPER.activation_function(self.hidden3(x))
        x = HYPER.activation_function(self.hidden4(x))
        x = HYPER.activation_function(self.hidden5(x))
