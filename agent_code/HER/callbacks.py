...
from .model import DQN, Memory, Transition, HYPER, ACTIONS
...
def setup(self):
    ...
    if self.train or not os.path.isfile("policy_net.pt"):
        ...
        self.target_model = DQN(HYPER.N_FEATURES, HYPER.N_ACTIONS).to(self.device)
        self.target_model.load_state_dict(self.model.state_dict()) # copy weights and stuffweights = np.random.rand(len(ACTIONS))
