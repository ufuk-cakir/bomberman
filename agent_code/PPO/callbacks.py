...
def act(self, game_state: dict) -> str:
    ...
    s = torch.from_numpy(s).float().to(self.device) if self.train else torch.from_numpy(s).float()
    with torch.no_grad():
        action,logprob,entropy, value = self.model.get_action_and_value(s)
        if self.train:
            self._obs= s.detach().cpu().numpy() if self.train else s.cpu().numpy()
            self._log_prob = logprob
            self._entropies=entropy
            self._value = value
            self._action = action
            
            # this is later used in the train function to push  
            #Rewards and Dones are set in the game_events_occurred function
